...
export function InstructorResources() {
  ...
  const [modal, setModal] = useState(false);
  const [form, setForm] = useState({ title: '', courseId: '', type: 'pdf', size: '' });
  const [confirmDelete, setConfirmDelete] = useState(null);

  const handleUpload = () => {
    if (!form.title || !form.courseId) { showToast('Fill in all fields', 'error'); return; }
    uploadResource({ ...form, instructorId: currentUser.id });
    setModal(false);
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Upload Resource">
        ...
        <div style={{ border: `2px dashed ${BRAND.gray200}`, borderRadius: 12, padding: 30, textAlign: 'center', marginBottom: 14, background: BRAND.gray50 }}>
          ...
          <p style={{ fontSize: 11, color: BRAND.gray300 }}>PDF, Video, ZIP, DOC (max 100MB)</p>
          <button className="btn btn-outline btn-sm" style={{ marginTop: 10 }}>Browse Files</button>
        </div>
